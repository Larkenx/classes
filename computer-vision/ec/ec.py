...
# Image.fromarray(uint8(canny_scene)).save('chamfer_scene_canny_doesn't_work.jpg')
# Image.fromarray(uint8(canny_search_object)).save('chamfer_object_canny_doesn't_work.jpg')
...
def distance_transform(canny_img):
    ...
    for i in range(canny_img.shape[0]-1, 0, -1): # not doing the last row
        for j in range(canny_img.shape[1]-1, 0, -1): # not doing the last column
            if d[i,j] == inf:
                if d[i+1, j] >= 0: # below
                    d[i,j] = d[i+1, j] + 1
                elif [i, j+1] >= 0: # right
